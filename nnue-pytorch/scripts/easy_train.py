...
LOCAL_NNUE_PYTORCH_PATH = r"D:\AI\nnue_pytorch"
...
def is_nnue_pytorch_setup(directory): # ĐỊNH NGHĨA TRƯỚC
    try:
...
def setup_nnue_pytorch(directory, repo, branch_or_commit):
    # 'directory' ở đây là nơi easy_train.py *muốn* thiết lập nnue-pytorch
    # ví dụ: D:\AI\nnue-pytorch\scripts\easy_train_data\experiments/experiment_test\nnue-pytorch

    # Kiểm tra xem nnue-pytorch đã được thiết lập chưa TẠI ĐÍCH ĐẾN
    if is_nnue_pytorch_setup(directory): # Hàm này kiểm tra bằng cách chạy nnue_dataset.py
        LOGGER.info(f'nnue-pytorch already setup in {directory}')
        return

    LOGGER.info(f'Setting up nnue-pytorch in {directory}')

    if LOCAL_NNUE_PYTORCH_PATH and os.path.exists(LOCAL_NNUE_PYTORCH_PATH):
        LOGGER.info(f'Using local nnue-pytorch from: {LOCAL_NNUE_PYTORCH_PATH}')
        # Xóa thư mục đích nếu nó tồn tại để đảm bảo copy sạch
        if os.path.exists(directory):
            shutil.rmtree(directory)
        # Copy toàn bộ nội dung từ thư mục cục bộ của bạn vào thư mục đích mà easy_train mong đợi
        shutil.copytree(LOCAL_NNUE_PYTORCH_PATH, directory)
        LOGGER.info(f'Copied local nnue-pytorch to {directory}')
    else:
        # Logic gốc: tải từ Git
        git_download_branch_or_commit(directory, repo, branch_or_commit)

    # Phần còn lại của hàm giữ nguyên để biên dịch data loader
    command = []
    ...
    with subprocess.Popen(command, cwd=directory) as process:
        if process.wait():
            raise Exception(f'nnue-pytorch {repo}/{branch_or_commit} data loader compilation failed (using {"local" if LOCAL_NNUE_PYTORCH_PATH and os.path.exists(LOCAL_NNUE_PYTORCH_PATH) else "git"})')

    if not is_nnue_pytorch_setup(directory):
        raise Exception(f'Incorrect nnue-pytorch setup or timeout (using {"local" if LOCAL_NNUE_PYTORCH_PATH and os.path.exists(LOCAL_NNUE_PYTORCH_PATH) else "git"}).')
